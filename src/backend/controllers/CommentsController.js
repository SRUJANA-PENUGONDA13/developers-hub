...
export const editPostCommentHandler = function (schema, request) {
  ...
  try {
    ...
    const { editComment, replyComment } = JSON.parse(request.requestBody);
    const post = schema.posts.findBy({ id: postId }).attrs;
...
export const deletePostCommentHandler = function (schema, request) {
  ...
  try {
    ...
    if (
      post.comments[commentIndex].user.username !== user.username &&
...
export const upvotePostCommentHandler = function (schema, request) {
  ...
  try {
    ...
    post.comments[commentIndex].votes = post.comments[commentIndex].votes + 1;
